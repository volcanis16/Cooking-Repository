class SearchController < ApplicationController
  ...
  ##Search refines the query by building individual queries and then merging them with previous queries
   #This creates one large query with unique aliases the continually reduces the possible results

  def search
    ##Split input into manageable arrays
    input_split = input_split(params[:search].downcase)
...
private
  ...
  def full_query(input, recipes)
    ...
    unless input[1].empty?
      ##Join with tables multiple times with unique aliases for each search term
    ...
    unless input[0].empty?
      ##Build an array of query ready terms and matching length where argument
  ...
  def cat_negative(input, recipes)
    ##Join with category table with unique aliases then get a list of any recipe not using that category
    ##Use merge to remove any of the recipes from the current list that aren't in the query result
    input.each_with_index do |name, idx|
  ...
  def tag_negative(input, recipes)
    ##Join with tag table with unique aliases then get a list of any recipe not using that category
    input.each_with_index do |name, idx|
